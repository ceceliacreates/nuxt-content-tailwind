...
<template>
    <main>
      ...
      <div>
        ...
        <Item v-for="event in displayedPastEvents" :key="event.id" :item="event" section="events" @update-selected-tag="filterByTag" />
      </div>
        <div class="text-center">
          <p class="italic font-light" v-if="atEnd">End of events</p>
          <button class="bg-blue text-white px-4 py-2 rounded" v-else @click="loadMoreEvents">Load More</button>
        </div>
    </main>
</template>